...
for i = 1:800
    ...
    original_data = [original_data; frame_data];    % 组合列向量
end
error_rate = 0.01;     % 设置误码率1%
error_mask = rand(length(original_data), 1) < error_rate;   % 根据误码率生成掩码
corrupted_data = xor(original_data, error_mask);            % 使用 XOR 操作引入误码

% 参数设置
bits = corrupted_data(20:end);      % 截获的比特流数据
